...
(async () => {
  ...
  if (hasReady) {
    // Second line of defense: actively reset animation time to zero — handles the case
    // where HTML doesn't strictly follow the starter tick template (e.g. lastTick uses
    // performance.now() so font load time gets counted into the first-frame dt).
    // See references/animation-pitfalls.md §12 for details.
    ...
    if (seekCorrected) {
      // Wait two rAFs so the seek takes effect and the t=0 frame is rendered
