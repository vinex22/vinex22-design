#!/usr/bin/env python3
"""
verify.py — Playwright wrapper for verifying HTML output from vinex22-design
...
Usage:
    ...
    python verify.py deck.html --slides 10                  # slide mode: capture first N slides
...
# Force UTF-8 stdout/stderr so the script works on Windows PowerShell
# (default cp1252 crashes on the ✓ ✗ → 📸 glyphs printed below).
# Safe no-op on Linux/macOS where stdout is already UTF-8.
...
def verify_html(html_path, viewports=None, slides=0, output_dir=None, show=False, wait=2000):
    ...
    with sync_playwright() as p:
        ...
        for viewport in viewports:
            ...
            print(f"\n→ open {file_url} @ {viewport['width']}x{viewport['height']}")
...
def main():
    ...
    parser.add_argument("--slides", type=int, default=0,
                        help="slide mode: capture first N slides (HTML must support ArrowRight paging)")
    ...
    parser.add_argument("--wait", type=int, default=2000,
                        help="ms to wait after opening the page (default 2000)")
