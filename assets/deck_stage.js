 ...
 * - Speaker notes postMessage (supports outer rendering)
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Render Shadow DOM first (independent of child nodes, immune to parser timing)
      this._render();

      // Defense: if the script is placed in <head> (not after </deck-stage>),
      // the parser may not have finished processing child <section>s yet, so querySelectorAll returns empty.
      // Defer to the next event loop tick to ensure all children are parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document still parsing — DOMContentLoaded picks up all sections in one go
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document parsed (script at bottom of body or defer); collect on next frame
